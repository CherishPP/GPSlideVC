...
- (void)buildUI
{
    NSUInteger number = [self.switchViewDelegate numberOfTab:self];
    for (int i=0; i<number; i++) {
        UIViewController *vc = [self.switchViewDelegate slideSwitchView:self viewOfTab:i];
    ...
    //选中第一个view
    if (self.switchViewDelegate && [self.switchViewDelegate respondsToSelector:@selector(slideSwitchView:didselectTab:)]) {
        [self.switchViewDelegate slideSwitchView:self didselectTab:_userSelectedChannelID - 100];
    }
...
-(void)scrollHandlePan:(UIPanGestureRecognizer*) panParam
{
    ...
    if(_rootScrollView.contentOffset.x <= 0) {
        if (self.switchViewDelegate
            && [self.switchViewDelegate respondsToSelector:@selector(slideSwitchView:panLeftEdge:)]) {
            [self.switchViewDelegate slideSwitchView:self panLeftEdge:panParam];
        }
    } else if(_rootScrollView.contentOffset.x >= _rootScrollView.contentSize.width - _rootScrollView.bounds.size.width) {
        if (self.switchViewDelegate
            && [self.switchViewDelegate respondsToSelector:@selector(slideSwitchView:panRightEdge:)]) {
            [self.switchViewDelegate slideSwitchView:self panRightEdge:panParam];
        }
    }
}
